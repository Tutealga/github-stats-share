...
const ProgressBar = ({ nextLevelPercentage, nextLevel, bg }) => {
  return (
    <div title={`${nextLevelPercentage > 100 ? 100 : nextLevelPercentage.toFixed(1)}%`} className={`h-[4px] w-full rounded-md ${bg} bg-opacity-20 flex items-center justify-between selection:ease-in-out duration-500 transition`}>
